...
// 图的算法都可以查得到,但是实现图的方式有一些不同的地方,并不存在万用的图,如果有只是牺牲了性能.
// 图有出度入度都存的,有只存一边要另一边在反转一次的.
// 稀疏图用邻接链表,密集图用二维矩阵.
// 如果没有涉及边算法可以存单工的边,因为另一个点可以作索引,但是涉及边的算法就要双工的点.
...
// edge from or to a vtx.
// simplex edge stores only one vertex.
type edge struct {
